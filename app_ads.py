...
date = sys.argv[1]
metric = sys.argv[2]
DURATION_DAYS = 29 # Always provide X - 1 Ex. Need for 5 days;  provide 4.

current_date = datetime.now().strftime("%Y-%m-%d")
ADS_DATA_NOT_AVAILABLE = False
dates_with_errors = []

logging.basicConfig(filename = f"logs/swiggy_ad_logs_{current_date}.log", 
                    level=logging.INFO,
...
def select_date(driver,date_to_select):
    ...
    input_element = wait_visible(driver,"//input[@placeholder='To']",10)

    from_input_element = wait_visible(driver,"//input[@placeholder='From']",10)
    ...
    def wait(frame=0):
        ...
        while True:
            ...
            if len(cells) > 0:
                ...
                printLog("--Data didn't load after 20 seconds.")
                return False
...
def process_ad_performance(driver,restaurant_ids,date):
    ...
    cur.execute("select * from swiggy_ads_month_track")
    ...
    for i in range(-1,DURATION_DAYS):
        ...
        date_to_scrape = (date + timedelta(days=i)).strftime('%Y-%m-%d')

        if date_to_scrape in scraped_dates:
            printLog("Date is already scraped. Skipping date.")
            continue
         
        check = select_date(driver,date_to_scrape)
        if not check:
            printLog("Date selection error. !!!SKIPPING DATE!!!")
            with open('ad_dates_with_error.txt','+a') as file:
                file.write(date_to_scrape)
                file.write('\n')
            continue
        try:
            refresh(driver)
        ...
        check = get_data(driver,date_to_scrape)
        if check:
            pass
            ...
            driver.refresh()
            select_date(driver,date_to_scrape)
            get_data(driver,date_to_scrape)
...
def process_discount_performance(driver,restaurant_ids,restaurants_names,phone_,date):
    ...
    for i in range(len(cities_)):
        ...
        for i in range(-1,DURATION_DAYS):
            ...
            if single_account:
                check = get_rev_single_res(driver,restaurant_ids[0],date_to_scrape)

                if not check:
                    get_rev_single_res(driver,restaurant_ids[0],date_to_scrape)

            else:
                check = get_data_rev(driver,date_to_scrape)

                if not check:
                    get_data_rev(driver,date_to_scrape)
...
def process_account(phone_,pass_):
    ...
    if int(metric) == 1:
        ...
        for _ in range(2):
            response = attempt(process_ad_performance,driver,restaurant_ids,date)
            if response:
                break
    else:
        ...
        for _ in range(2):
            response = attempt(process_discount_performance,driver,restaurant_ids,restaurants_names,phone_,date)
            if response:
                break
